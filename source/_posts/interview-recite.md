...
事务提交时，从buffer pool写入redo log buffer，按时间/事务提交刷新到redo log file。
...
加法增加乘法减少(AIMD)窗口算法
![tcp.png](https://iota-1254040271.cos.ap-shanghai.myqcloud.com/image/tcp.png)
...
链接：https://www.nowcoder.com/questionTerminal/51123ddacab84a158e121bc5fe3917eb?toCommentId=23586%E6%9D%A5%E6%BA%90%EF%BC%9A%E7%89%9B%E5%AE%A2%E7%BD%91
...
### 9.字典
字典dict 一般情况下， 字典只使用 ht[0] 哈希表， ht[1] 哈希表只会在对 ht[0] 哈希表进行 rehash 时使用。
```c
typedef struct dict {
...
其中哈希表是 指向 dict.h/dictEntry 结构的指针
```c
...
### 删除正在读写的文件
`lsof -n | grep deleted` 然后`kill -9 PID`
