...
存在的意义：
如果A表获取了某行行排他锁，顺便获取了表的意向排他锁。
A未提交，B获取不到表的意向共享锁，不用检查各个页/行的锁，只需要检查表锁。
...
MVCC读不加锁，读写不冲突。
https://t.hao0.me/mysql/2016/09/10/mysql-innodb-03-lock-and-transaction-model.html
...
6）索引列上使用表达式or函数 时期类不能用`to_days`
...
### TCP 流量控制
接收窗口为0
发送方暂停发送的状态将持续到主机B重新发出一个新的窗口值为止
TCP连接的一方收到对方的零窗口通知，就启动持续计时器。若持续计时器设置的时间到期，就发送一个零窗口控测报文段（携1字节的数据），那么收到这个报文段的一方就重新设置持续计时器。
...
- 不剥夺条件：进程已获得的资源，在末使用完之前，不能强行剥夺，只能自愿放弃。
- 循环等待条件：若干进程之间形成一种头尾相接的循环【等待资源】关系。
...
3）堆
4）方法区
5）运行时常量池：从方法区分配，类加载到虚拟机创建运行时常量池
6）本地方法栈


#### 介绍JVM堆和栈，有什么用
虚拟机栈：生命周期与线程相同。Java 方法执行的内存模型，
栈帧：1）局部变量表、返回地址、操作数栈、2）动态链接3）异常分派
动态链接：指向当前方法所在类的运行时常量池的引用。
    class文件里，方法调用其它方法/访问成员变量，需要符号引用。动态链接：将符号引用转换为对实际方法的直接引用。
    类加载：解析：将常量池里的符号引用->直接引用
异常分派
...
对象的表示：一个句柄的引用，句柄包括2个指针
1）表格：对象各个方法、Class对象指针。2）指向堆中对象的实例数据
...
### 8.Spring AOP 和 Spring Bean 生命周期如何结合
...
例子1.：
大批量数据的删除/更新，每次更新5k条，暂停几秒给主从同步
写一个存储过程

例子2：
not in <> 的子查询优化 -> left join + is null
查询没有付款的用户信息。
```sql
